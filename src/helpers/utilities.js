...
export function searchByText(items, column, searchQuery) {
  if (!isEmpty(searchQuery.dictionary)) {
    return filter(items, (item) =>
      searchQuery.dictionary.includes(item[column])
    );
  }
  ...
  switch (options) {
    case "all":
      if (searchQuery.value) {
        return filter(
          ...
          (item) =>
            item[column] &&
            item[column].toLowerCase().includes(searchQuery.value.toLowerCase())
        );
      }
      return items;
    case "equal":
      return filter(
        items,
        (item) => item[column] && item[column] === searchQuery.value
      );
    case "notEqual":
    ...
    case "match":
      return filter(
        items,
        (item) => item[column] && item[column].match(searchQuery.value)
      );
    case "notMatch":
      return filter(
        items,
...
export function searchByBoolean(items, column, searchQuery) {
  ...
  switch (options.toString()) {
    ...
    case "empty":
      return filter(
        items,
        (item) => item[column] !== true && item[column] !== false
      );
    case "notEmpty":
      return filter(
        items,
  ...
  if (searchQuery.value) {
    if (searchQuery.value.from && searchQuery.value.to) {
      return filter(items, (item) => {
        ...
        return (
          itemValue &&
          itemValue >= searchQuery.value.from &&
          itemValue <= searchQuery.value.to
        );
      });
    }
    if (searchQuery.value.from) {
      return filter(
        items,
        (item) => item[column] && item[column] >= searchQuery.value.from
      );
    }
    if (searchQuery.value.to) {
      return filter(
        items,
        (item) => item[column] && item[column] <= searchQuery.value.to
      );
    }
  }
  return items;
...
export const searchByOr = (items, schema, searchQuery) => {
  ...
  Object.entries(searchQuery).map(([column, searchQueryValue]) => {
    const newFilteredItems = searchByType(
      items,
      schema[column].type,
      column,
      searchQueryValue
...
export const queryDataType = (query, schema) => {
  const dateTimeAttributes = Object.keys(
    filterSchemaByType(schema, "datetime")
  );
  const durationAttributes = Object.keys(
    filterSchemaByType(schema, "duration")
  );
